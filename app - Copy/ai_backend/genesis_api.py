...
class GenesisAPI:
    ...
    def __init__(self):
        ...
        Sets the backend status to not running and clears the recorded start time.
        """
    ...
    async def shutdown(self):
        ...
        After shutdown, the API's running status is set to False. Errors encountered during shutdown are logged.
        """
        try:
...
def chat_with_genesis():
    ...
    Expects a JSON payload with `message` and `user_id` fields, and an optional `context` object. Responds with HTTP 400 for invalid or incomplete requests, and HTTP 500 for internal errors.
...
def internal_error(error):
    ...
    Returns:
        tuple: A JSON response with error details and the HTTP 500 status code.
    """
    return jsonify({
        "error": "Internal server error",
...
def initialize_app():
    """
    Asynchronously initializes the Genesis backend before processing the first incoming client request.
    """
